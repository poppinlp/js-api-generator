...
module.exports = function (options) {
    var path = require('path'),
        ...
        inputFilePath = options.target.charAt(0) === '/' ? options.target : path.join(pwd, options.target),
...
\
    ...
    if (options.outputFile) {
        fs.writeFileSync(
            options.outputFile.charAt(0) === '/' ? options.outputFile : path.join(pwd, options.outputFile),
            tpl, encoding);
    } else {
        return tpl;
    }
